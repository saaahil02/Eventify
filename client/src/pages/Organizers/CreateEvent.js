...
const CreateEvent = () => {
  ...
  const [activeTab, setActiveTab] = useState('create');
  const [isSubmitted, setIsSubmitted] = useState(false);
  const [eventSponser, setEventSponser] = useState('No'); // State to track event sponsor selection
  ...
  const handleFinish = async (values) => {
    ...
    try {
      ...
      formData.append('eventCategory', values.eventCategory);
      formData.append('eventSponser', values.eventSponser);
      formData.append('SponserDescription', values.eventSponserDescription);
      formData.append('eventMaxParticipants', values.eventMaxParticipants);
      formData.append("questions", JSON.stringify(questions));

      if (values.eventSponser === 'Yes') {
        formData.append('eventSponserDescription', values.eventSponserDescription);
      }
  ...
  return (
    <OrgLayout>
      <Form layout="vertical" onFinish={handleFinish}>
        ...
        <Form.Item label="Event Sponser" name="eventSponser" rules={[{ required: true, message: "Requires Sponser For Event" }]}>
          <Select placeholder="Required Sponser" onChange={(value) => setEventSponser(value)}>
            <Option value="Yes">Yes</Option>
            <Option value="No">No</Option>
          </Select>
        </Form.Item>
        ...
        {eventSponser === 'Yes' && (
          <Form.Item label="Sponser Description" name="eventSponserDescription" rules={[{ required: true, message: "Please enter the sponsor description" }]}>
        ...
        <Form.Item>
          <Checkbox checked={isFormSelected} onChange={(e) => setIsFormSelected(e.target.checked)}>
            Add Custom Form (Optional)
          </Checkbox>
        </Form.Item>
        ...
        <hr />
        <div className="text-center">
          <h2>Custom Filling Form</h2>
          <Modal title="Custom Form" visible={isModalVisible} onCancel={() => setIsModalVisible(false)} footer={null}>
            <div className="App">
              <h1>Custom Filling Form</h1>
              <Navbar filterItem={handleTabChange} />
